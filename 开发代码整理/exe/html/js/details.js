...
var host1 = 'http://39.97.180.149:83/zh/fastadmin/public/'; //域名
var app = new Vue({
    el: "#app",
    data: {
        List: {
                ...
                */
                ...
                if (img) {
                    ...
                    for (var i = 0; i < files.length; i++) {
                        ...
                        var imgSrcI = _this.getObjectURL(files[i]);
                        _this.imgName.push(files[i].name);
                        _this.imgSrc.push(imgSrcI);
                        _this.imgFile.push(files[i]);

                    }
                    _this.addNewContent(imgBox)
                    console.log(uploadFile, img, 8888)
                    if ("undefined" != typeof (uploadFile) && uploadFile != null && uploadFile != "") {
                        ...
                        $.ajax({
                            ...
                            success: function (res) {
                                ...
                                // objarr = res.data;
                                _this.uploadImg.push(arr1);
                                var listimgs = _this.flatten(_this.uploadImg)
                                // arrlist.push(arr1)
                                console.log('---------------------------')
                                console.log(arr1, 0)

                                if (listimgs.length) {
                                    for (var i = 0; i < _this.returnimgs.length; i++) {
                                        var obj2 = {};
                                        obj2.index = i;
                                        obj2.thumb = _this.returnimgs[i]
                                        arr2.push(obj2)
                                    }
                                }
                                _this.returnimgs = arr2;
                                console.log(_this.returnimgs, '最后值 ')
                                console.log('---------------------------')
                            },
                            error: function () {
                                alert("上传失败！");
...
function removeImg(obj, index) {
    var deleteIndex = $(obj).parent().data('index');
    alert(deleteIndex)
    // 索引对应对象索引
    var selfImg = app.uploadImg;
    // console.log(selfImg, 999999999999999)
    var arr1 = [];
    for (var y = 0; y < selfImg.length; y++) {
        var obj1 = {};
        if (selfImg[y].index == deleteIndex) {
    ...
    app.uploadImg = arr1;
    app.imgSrc.splice(index, 1);
    app.imgFile.splice(index, 1);
    app.imgName.splice(index, 1);
    console.log(app.uploadImg, 0000000000)
    $(this).parent().remove();

    app.addNewContent('#gallery');

    // console.log(app.imgSrc, 1)

    // var boxId = "#" + $(obj).parent('.img-item').parent().attr("id");
    // app.addNewContent(boxId);
}
